...
def train_model():
    model = Model(class1.get(), class2.get(), feature1.get(), feature2.get(), float(learning_rate.get()), number_of_epochs.get(), bias.get())
    weights = model.train() 
...
number_of_epochs_entry.grid(row=11, column=10)

# Bias
Label(master, text="Bias", font=25).grid(row=13, column=1, padx=10, pady=10)
bias = BooleanVar(value=True)
Radiobutton(master, text="YES", variable=bias, value=True).grid(row=13, column=5)
Radiobutton(master, text="NO", variable=bias, value=False).grid(row=13, column=10)
